...
- **If the skill runs an ML model (image/audio/video generation, local inference with heavy deps), use `create-generative-skill` instead** — it covers model_spec, host adapters, and the download + coordinator-restart flow.
